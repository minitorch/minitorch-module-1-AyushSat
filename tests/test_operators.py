...
@pytest.mark.task0_2
@given(small_floats)
def test_neg(a: float) -> None:
    """Write a test that ensures that a float is equal to the float's negative's negative"""
    assert eq(a, neg(neg(a)))


@pytest.mark.task0_2
@given(small_floats)
def test_is_close(a: float) -> None:
    """Write a test that ensures that a float is close enough to the float + 1e-4"""
    assert is_close(a, a + 1e-4)
